...
class AppForm(QMainWindow):
    ''' Main application GUI form for scatter plot.
    ...
    >>> sys.exit(app.exec_())
    '''
    def __init__(self, parent=None, x_file='x.npy', y_file='y.npy', wait_for_file_close=.01):
        ...
        self.mtime_x = os.path.getmtime(self.x_file)
        self.mtime_y = os.path.getmtime(self.y_file)
        self.watcher.addPath(self.x_file)
        self.watcher.addPath(self.y_file)
    ...
    def _on_file_changed(self):
        ...
        newtime_x = os.path.getmtime(self.x_file)
        newtime_y = os.path.getmtime(self.y_file)
        if self.mtime_x < newtime_x:
            self.mtime_x = newtime_x
            time.sleep(self.wait_for_file_close)
            self.x = np.load(self.x_file)
        if self.mtime_y < newtime_y:
            self.mtime_y = newtime_y
            time.sleep(self.wait_for_file_close)
            self.y = np.load(self.y_file)
        self.scatterplot.on_draw(self.x, self.y)
