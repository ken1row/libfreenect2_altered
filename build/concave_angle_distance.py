...
def valid_l2_norm(vec1, vec2, ave=False):
    ...
    if not ave:
        ...
        return sum(l2 * valid) / sum(valid) / vec1.shape[1] * 20
    
    
def plot_angle_distance(base='paper_concave_angle_test12', targ='paper_concave_angle_test', angles=range(60, 301, 10)):
    import pylab
    ...
    probe = np.vstack(load_data(base, absolute_depth=False, linear_stage=False, relative_120=False, normalize_metric=False)).T
    dv = []
    for idx, angle in enumerate(angles):
        dname = targ + str(idx).zfill(2)
        test = np.vstack(load_data(dname, absolute_depth=False, linear_stage=False, relative_120=False, normalize_metric=False, guarantee=200)).T
        dv.append(max(0, valid_l2_norm(probe, test, ave=True)-3))
    pylab.figure(figsize=(6, 2))
#    dv = dv[::-1]
    pylab.plot(angles, dv, lw=2)
    limit = np.ones(len(dv)) * 4
    pylab.plot(angles, limit, lw=2, c='r')
    pylab.ylim((0, 15))
    pylab.xlim((58, 245))
    pylab.xlabel('Angle of opening [degree]', fontproperties=times)
    pylab.ylabel('Distance [mm]', fontproperties=times)
    pylab.xticks(range(60, 241, 30), range(60, 241, 30), fontproperties=times)
    pylab.yticks(range(0, 16, 5), range(0, 16, 5), fontproperties=times)
#    pylab.yticks(range(0, 3, 1), range(0, 3, 1), fontproperties=times)
    pylab.savefig('fig/concave_angle_distance.pdf', bbox_inches='tight', pad_inches=0.1)
    
plot_angle_distance()#base='cardboard00')